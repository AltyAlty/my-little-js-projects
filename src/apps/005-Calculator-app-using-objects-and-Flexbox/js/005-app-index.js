const classes = {
    enterNumberAndDotButtonClass: 'enter-number-and-dot-button', /*Это класс, указывающий на кнопки c цифрами 0-9 и
    операцией ".".*/
    calcButtonOneClass: 'calc-button-one', /*Это класс, указывающий на кнопки операций в первом ряду
    ("+", "-", "*", ":").*/
    calcButtonTwoClass: 'calc-button-two', /*Это класс, указывающий на кнопки операций во втором ряду
    ("+/-", "=", "С").*/
    ...
    currentFirstNumberIsClass: 'first-number-is', /*Это класс, указывающий на элемент, отображающий текущее введенное
    первое число снизу в дополнительной информации.*/
    currentOperationIsClass: 'current-operation-is', /*Это класс, указывающий на элемент, отображающий текущую выбранную
    операцию снизу в дополнительной информации.*/
    currentSecondNumberIsClass: 'second-number-is' /*Это класс, указывающий на элемент, отображающий текущее введенное
    второе число снизу в дополнительной информации.*/
};
...
const calculator = {
    ...
    /*Создаем вспомогательные переменные для хранения текущих введенных первого и второго чисел, а также текущей
    выбранной операции.*/
    currentFirstNumber: '',
    currentSecondNumber: '',
    currentOperation: '',
    ...
    /*Функции для отображения информации об участниках текущей операции.*/
    showInfo: function () {
    ...
    /*Это функция-подписчик для ввода данных в поле вывода в калькуляторе. Эта функция принимает какое-то значение и
    ...
    enterNumberAndDotButtonListener: function (event) {
        ...
        /*Далее мы получаем элемент события и сохраняем у этого элемента свойства "value" в качестве данных для ввода
        в поле вывода.*/
    ...
    /*Это функция-подписчик, которая после выбора операции сохраняет текущее введенное первое число и текущую выбранную
    операцию.*/
    calcButtonOneListener: function (event) {
        ...
        проверяем, что у нас имеется текущее выбранное первое число и текущая выбранная операция, а также то, что поле
        для вывода в калькуляторе не пустое и не состоит из одной точки, знака минус или знака минус с точкой, чтобы
        ...
        числа для текущей операции, то мы сначала вычисляем текущую операцию. Если текущее введенное первое число было
        сброшено, что означает успешное завершение операции, то результат текущей операции сохраняем как первое число
        для следующей операции, затем последнюю выбранную операцию устанавливаем как новую текущую выбранную операцию и
        зануляем поле для вывода, чтобы подготовить его для ввода второго числа в новой текущей операции. И в конце
        выводим информацию об участниках новой текущей операции.*/
        ...
        } else { /*Иначе если у нас нет еще невысчитанных операций, то есть мы имеем только текущее введенное первое
        число, то мы просто сохраняем текущую выбранную операцию.*/

            /*Сначала проверяем, что текущего введенного числа нет и, что поле для вывода в калькуляторе не пустое и не
            состоит из одной точки, знака минус или знака минус с точкой. Если проверка проходит, то мы сохраняем то,
            что в данный момент введено в поле, в качестве введенного первого числа, а затем зануляем поле для вывода в
            калькуляторе.*/
            if (
            ...
            /*Затем проверяем, что текущего введенного числа нет и, что поле для вывода в калькуляторе пустое, состоит
            из одной точки, знака минус или знака минус с точкой. Если проверка проходит, то мы в качестве введенного
            первого числа указываем нуль, а затем зануляем поле для вывода в калькуляторе.*/
            if (
                calculator.currentFirstNumber === ''
                && (
                    calculator.elements.resultElement.innerText === '' || calculator.elements.resultElement.innerText === '.'
    ...
    removeSomethingListener: function () {
        ...
        /*Затем пробегаемся по строке и проверяем осталась ли там точка. Если точка была удалена, то мы должны указать,
        что точки в числе мы на данные момент больше не имеем. Если не делать эту проверку, то удалив точку мы не сможем
        ее поставить еще раз.*/
        for (let i = 0; i < calculator.elements.resultElement.innerText.length; i++) {
            if (calculator.elements.resultElement.innerText[i] !== '.') {
                calculator.ifWeHaveDot = false;
            };
        ...
        switch (operation) {
            ...
            case '+/-':
                ...
                if (calculator.elements.resultElement.innerText[0] !== '-') {
                    ...
                    /*Строки в JS "immutable", поэтому мы не можем просто изменить какой-то один символ в строке, то
                    есть нельзя сделать так "resultElement.innerText[0] = '123'", нам нужно переназначить измененную
                    версию обратно в "innerText".*/
                    let content = calculator.elements.resultElement.innerText;
                    /*Метод "substring()" возвращает подстроку строки между двумя индексами, или от одного индекса и до
                    конца строки.*/
            ...
            case '=':
                /*Сначала проверяем, что у нас есть текущее введенное первое число и поле для вывода не является пустым,
                не состоит только из точки, знака минуса или знака минуса с точкой. Если это так, то сохраняем то, что
                введено в данный момент в поле для вывода, в качестве текущего введенного второго числа. Иначе это
                второе число оставляем пустым.*/
                if (
                    calculator.currentFirstNumber !== ''
                    && calculator.elements.resultElement.innerText !== '' && calculator.elements.resultElement.innerText !== '.'
                    && calculator.elements.resultElement.innerText !== '-' && calculator.elements.resultElement.innerText !== '-.'
                ...
                /*Теперь когда у нас есть текущие введенное первое число, введенное второе число и операция, мы можем
                начать рассчет.*/
                if (calculator.currentFirstNumber !== '' && calculator.currentSecondNumber !== '' && calculator.currentOperation !== '') {
                    ...
                    switch (calculator.currentOperation) {
                        /*Если у нас операция сложения, то проверяем, что ни одно из чисел не больше 15 знаков, так как
                        JS не может нормально работать с большими числами. Если проверка не проходит, то сообщаем об
                        этом, иначе находим количество разрядов в десятичной части, производим рассчет, результат
                        ...
                        /*Если у нас операция вычитания, то проверяем, что ни одно из чисел не больше 15 знаков, так как
                        JS не может нормально работать с большими числами. Если проверка не проходит, то сообщаем об
                        этом, иначе находим количество разрядов в десятичной части, производим рассчет, результат
                        указываем в поле для вывода, выводим информацию об участниках операции, а затем делаем их
                        сброс.*/
                        ...
                        умножение таких чисел даст 16-ти значное число, а JS не может нормально работать с большими
                        числами, и также проверяем, что ни одно из чисел не равно нулю, так как произведению с нулем
                        всегда дает нуль. Если проверка не проходит, то сообщаем об этом, иначе находим количество
                        разрядов в десятичной части, производим рассчет, результат указываем в поле для вывода, выводим
                        информацию об участниках операции, а затем делаем их сброс.*/
                        case '*':
                        ...
                        /*Если у нас операция деления, то проверяем, что ни одно из чисел не больше 15 знаков, так как
                        JS не может нормально работать с большими числами. Если проверка не проходит, то сообщаем об
                        этом, иначе если второе число не является нулем, то производим рассчет, результат указываем в
                        поле для вывода, выводим информацию об участниках операции, а затем делаем их сброс. Если же
                        второе число было нулем, то выводим информацию об этом.*/
                        case ':':
                            if (calculator.currentFirstNumber > 999999999999999 || calculator.currentSecondNumber > 999999999999999) {
                                alert('One of the numbers is more than 999 999 999 999 999!');
                                calculator.showInfo();
    ...
    findPlacesForRounding: function (numberOne, numberTwo, operation) {
        ...
        /*Задаем переменную, обозначающую количество разрядов после точки в результате в операциях сложения и вычитания,
        а также схожую переменную только для операции умножения.*/
        let places = 0;
        let placesForMult = 0;
        ...
        if (ifWeHaveDotInNumberOne || ifWeHaveDotInNumberTwo) {
            ...
            /*Находим строковые представления дробных частей первого и второго чисел в зависимости от того, какие из
            этих чисел имеют дробные части. Также здесь находим количество разрядов в дробной части в операции умножения
            путем сложения количества разрядов двух дробных частей.*/
            if (ifWeHaveDotInNumberOne && ifWeHaveDotInNumberTwo) {
            ...
            больше разрядов, находим разницу в количестве разрядов, и благодаря этой разнице дописываем нули в той
            дробной части, которая содержит меньше разрядов.*/
            ...
            отбросить нули только после этой цифры, а не до нее. Используется дальше.*/
            ...
            разрядов в дробной части результата, чем любая из дробных частей чисел в операции. Например, сложение "1.25"
            и "2.35" даст "3.6", или вычитание "2.35" и "1.25" даст "1.1". На такой случай мы пробегаем по строковому
            представлению результата сложения или вычитания дробных частей с конца в начало и проверяем наличие нулей в
            конце, так как при выводе нам они не будут нужны, то есть мы должны вывести "3.6", а не "3.60", или "1.1", а
            не "1.10".*/
            for (let i = 0; i < stringDecimalResult.length; i++) {
                /*Если мы нашли цифру отличную от нуля, то мы увеличиваем количество разрядов в дробной части на 1, и
                также отдельно отмечаем, что нашли такую цифру, чтобы в дальнейшем попадать во второй блок этого
                ...
                if (stringDecimalResult[stringDecimalResult.length - 1 - i] !== '0') {
                    ...
                    ++places;
                }; /*То есть, если строковое представление результата сложения или вычитания дробных частей у нас
                получилось "1030", то мы проигнорируем последний нуль, а "103" даст нам в сумме только 3 разряда в
                дробной части.*/
            };
            ...
            если сложить "0.78" и "0.42", то мы получим "1.2". Но в нашей программе при сложении дробных частей "78" и
            "42" мы получаем "120", где в реальном примере "100" перешло бы в целые числа, а оставшееся "20" стало бы
            "0.2". В таком случае дробная часть в результате будет по количеству разрядов меньше, чем любая из дробных
            частей чисел в операции. На такой случай мы проверяем больше ли по количеству разрядов дробная часть
            результата, чем одна из дробных частей чисел в операции, и если это так, то уменьшаем количество разрядов в
            дробной части результата на 1, чтобы избавиться в конце от лишнего нуля. То есть при сложении "0.78" и
            "0.42" мы получим дробные части "78" и "42", сложим их и получим "120", далее определим, что последний нуль
            лишний при подсчете разрядов дробной части и получим, что дробная часть должна иметь 2 разряда, когда в
            действительности ей нужен только один разряд - "1.2", а не "1.20". Поэтому здесь мы проверим, что строка
            "120" больше чем строка "78" или "42", поэтому уменьшим количество разрядов в дробной части до 1.
            
            Можно рассмотреть похожий случай с вычитанием, например вычтем из "2.35" число "1.05". Вычитаем дробные
            части "35" и "5", получаем "30". Здесь также получается, что последний нуль будет лишним в дробной части и
            по логике выше мы от него в итоге избавимся при учете количества разрядов в дробной части результата, то
            есть получим "1.3", а не "1.30".
            
            Отдельная проблема может быть при вычитании. Если мы вычтем из "1" число "8.1", то мы получим "-7.1". Но в
            нашей программе при вычитании дробных частей "0" и "1" мы получаем "-1", где в реальном примере единица была
            бы вычтена из целой части, а оставшееся "1" стало бы "0.1". В таком случае строковое представление дробной
            части будет длиннее, чем любая из дробных частей чисел в операции, так как будет содержать знак минуса. На
            такой случай мы проверяем больше ли по количеству разрядов дробная часть результата, чем одна из дробных
            частей чисел в операции, и если это так, то уменьшаем количество разрядов в дробной части результата на 1,
            чтобы избавиться в конце от лишнего нуля. То есть при вычитании "1" и "8.1" мы получим дробные части "0" и
            "1", вычтем их и получим "-1", далее определим, что дробная часть должна иметь 2 разряда, когда в
            действительности ей нужен только один разряд - "-7.1", а не "-7.10". Поэтому здесь мы проверим, что строка
            ...
            количества разрядов в дробной части и по логике выше мы от него в итоге избавимся при учете количества
        ...
        Иначе количество разрядов в дробной части в умножении не нулевых чисел будет равно "placesForMult". Иначе если у
        нас было сложение или вычитание, то количество разрядов в дробной части будет равно "places".*/
    ...
    initializeListeners: function () {
        ...
        /*Здесь мы пробегаемся по всем нашим кнопкам с операциями "+", "-", "*", ":" и подписываем их на событие
        "click", при срабатывании, которого будет вызываться функция "calcButtonOneListener()".*/
        ...
        /*Подписываем нашу кнопку удаления данных на событие "click", при срабатывании, которого будет вызываться
