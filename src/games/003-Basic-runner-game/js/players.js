...
function Player(
    ...
    this.predictCloseCollision = function () {
        ...
        /*Проверяем не будет ли коллизии между игроком и стенами по X в текущем тике и коллизии между игроком и стенами
        по Y в следующем тике.*/
        for (let i = 0; i < walls.length; i++) { // Перебираем каждую стену.
            ...
            if (helper.checkIntersectionBetweenTwoNotRotatedRectangles(
                ...
                /*Если такая коллизия есть, то пока такая коллизия имеет место, сдвигаем по Y предполагаемую проекцию
                игрока, которая будет в следующем тике, ближе к текущей позиции игрока на 1 до тех пор, пока не пропадет
                коллизия между предполагаемой позицией игрока и какой-то стеной. Делаем это пока игрок активный.*/
                while (helper.checkIntersectionBetweenTwoNotRotatedRectangles(
                    ...
                    if (this.isActive) {
                        ...
                        predictedVerticalPosition.y -= 1; // Сдвигаем на 1 вручную, так как когда игрок не активен, его
                        // скорость по Y равна 0.
                    };
                };

                /*Как только мы перестанем сдвигать по Y предполагаемую проекцию игрока, которая будет в следующем тике,
                ближе к текущей позиции игрока, то это будет означать, что мы имеем самую близкую позицию игрока для
                следующего тика, когда игрок будет касаться какой-то стены, но не проходит через нее. Поэтому указываем,
                что координата Y этой предполагаемой позиции должна быть текущей координатой Y игрока и останавливаем
                игрока по Y, чтобы он не пытался двигаться дальше в стену, так как это приведет к бесконечной работе
                цикла "while".*/
    ...
    this.predictFarCollision = function () {
        ...
        if (world.worldSpeed > 0) { /* Если будет двигаться вправо. Теперь проверяем в зависимости от скорости прокрутки
        мира, а не от собственной скорости игрока по X, так как он больше сам не двигается по X.*/
        ...
        /*Создаем переменные, хранящие расстояния до препятствий, которые могут потенциально оказаться на пути,
        пройденным игроком за следующий тик.*/
        ...
        if (world.worldSpeed > 0 && closestCollisionRight) { // Если мир прокручивается и игрок движется вправо и на его пути потенциально есть препятствия,
            ...
            /*то сдвигаем все стенки на расстояние этой коллизии, чтобы они перескочили через игрока.*/
    ...
    this.move = function () {
        if (this.isActive) { // Реализуем движение игрока, если он активен.
            ...
            if (this.currentSpeedY < this.gravity && this.currentAccelerationY !== 0) { // Если мы прыгаем,
                ...
                /*Если находясь в воздухе, расстояние, которые мы в нем прошли, больше установленного ограничения, то мы
                запрещаем игроку дальше прыгать и сбрасываем расстояние, пройденное нами в воздухе во время прыжка,
                чтобы в следующем прыжке начать отсчет сначала.*/
            ...
            /*Если игрок коснулся какой-то стенки своей нижней стороной, то даем возможность снова прыгать игроку и если
            мы сделали хотя бы небольшой прыжок и опустились на поверхность какой-то стены, то сбрасываем расстояние,
            которое прошли воздухе во время прыжка, чтобы в следующем прыжке начать отсчет сначала.*/
            ...
            /*Ограничиваем скорость по Y при достижении максимума.*/
            ...
            /*Округляем скорость по Y до целых значений. Поскольку метод "Math.floor()" для отрицательных значений,
            например, "-5,3" превращает в "-6", то есть модуль числа по факту округляется сверху, то для отрицательных
            значений используем "Math.ceil()".*/
    ...
    this.drawGIF = function () {
        ...
        if (this.isActive) {
            ...
            /*Если мы отрисовали последний кадр из раскадровки анимации, то возвращаемся на позицию X так, чтобы
            следующий кадр оказался самым первым из раскадровки. Иначе, двигаемся по раскадровке дальше с указанием во
            сколько раз должна быть медленнее отрисовка анимации, чем скорость работы игры.*/
        ...
        } else { /*Если игрок неактивный, то не отрисовываем анимацию бегущего игрока и сбрасываем данные,
        использующиеся для анимации игрока.*/
    ...
    this.drawJumpBar = function () {
        /*Если мы в воздухе и у нас больше нет возможности прыгать еще выше, то отрисовываем всю полоску красной.*/
        if (this.currentAccelerationY === 0 && this.jumpedDistance === 0) {
            ctx.fillStyle = 'red';
            ctx.fillRect(10, 14, this.maximumJumpedDistance, 30);
        } else { /*Если мы в воздухе и все еще можем прыгать, то отрисовываем оранжевым цветом определенную часть
        полосы.*/
