const world = {
    ...
    differenceBetweenCurrentLastWallAndNewWall: worldDefaultSettings.differenceBetweenCurrentLastWallAndNewWall,
    ...
    addFutureWalls: function (
        ...
        minimumWallHeight,
        differenceBetweenCurrentLastWallAndNewWall,
        newWallLowHeightEnhancer,
        firstWallWithHoleID,
        addingHoleDivisor,
        minimumHoleWidth
    ) {
        /*Если уже есть данные для как минимум 4 стен, то ничего не делаем.*/
        ...
        /*Если так получилось, что максимальная высота, на которую может запрыгнуть игрок c текущей последней стены
        оказалась больше, чем предустановленное ограничение на максимальную высоту стены, то мы делаем так, чтобы
        максимальная высота, на которую может запрыгнуть игрок c текущей последней стены была равна этому ограничению,
        чтобы случайно не создать стену, на которую не будет возможности запрыгнуть.*/
        if (biggestJumpableHeight > this.maximumWallHeight) {
            biggestJumpableHeight = this.maximumWallHeight;
        };

        /*Генерируем высоту для следующей стены. Полученная высота будет в диапазоне от минимально указанной высоты, до
        максимальной высоты, на которую игрок может запрыгнуть игрок c текущей последней стены, плюс минимально указанная
        высота.*/
        let newWallHeight = Math.floor(Math.random() * biggestJumpableHeight) + minimumWallHeight;

        /*Если текущая последняя стена ниже, чем следующая стена, которую мы пытаемся создать, и их разница по высоте
        меньше установленного ограничения, то увеличиваем высчитанную высоту для следующей стены.*/
        if (currentLastWall.height < newWallHeight && Math.abs(newWallHeight - currentLastWall.height) < differenceBetweenCurrentLastWallAndNewWall) {
        ...
        /*Если текущая последняя стена имеет id больше указанного показателя, id текущей последней стены кратен
        указанному показателю и текущая последняя стена не имеет перед собой пропасти, то */
    ...
    cleanOldWalls: function () {
        /*Мы не видим какой-либо стены, если она полностью ушла за экран. Это значит, что ее координата X как минимум
        равна отрицательному значению собственной ширины.*/
    ...
    increaseWorldSpeed: function (divisor) {
        /*Мы не видим какой-либо стены, если она полностью ушла за экран. Это значит, что ее координата X как минимум
        равна отрицательному значению ширины собственной ширины.*/
        for (const i in walls) {
            if (walls[i].x <= -walls[i].width) {
    ...
    /*Метод для отрисовки горизонтальной границы середины экрана.*/
