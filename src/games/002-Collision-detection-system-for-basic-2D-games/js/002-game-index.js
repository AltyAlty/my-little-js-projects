...
function Player(
    ...
    this.predictCollision = function () {
        ...
        for (let i = 0; i < walls.length; i++) { // Перебираем каждую стену.
            ...
            if (helper.checkIntersectionBetweenTwoNotRotatedRectangles(
                ...
                /*Если такая коллизия есть, то пока такая коллизия имеет место, сдвигаем по X предполагаемую проекцию
                игрока, которая будет в следующем тике, ближе к текущей позиции игрока на 1 до тех пор, пока не пропадет
                коллизия между предполагаемой позицией игрока и какой-то стеной.*/
                ...
                /*Как только мы перестанем сдвигать по X предполагаемую проекцию игрока, которая будет в следующем тике,
                ...
                что координата X этой предполагаемой позиции должна быть текущей координатой X игрока и останавливаем
                игрока по X, чтобы он не пытался двигаться дальше в стену, так как это приведет к бесконечной работе
                цикла "while".*/
        ...
        /*Подготавливаем данные, описывающие путь, который игрок может пройти за следующий тик, если будет двигаться по
        X.*/
    ...
    this.move = function () {
        if (this.isActive) { // Реализуем движение игрока, если он активен.
            ...
            /*Ограничиваем скорость по X при достижении максимума.*/
            ...
            /*Округляем скорость по X до целых значений. Поскольку метод "Math.floor()" для отрицательных значений,
            ...
            if (this.speedX > 0) { // Если скорость по X больше 0,
                ...
                this.speedX = Math.ceil(this.speedX); // Если скорость по X меньше или равна 0, то округляем скорость по
                // X сверху.
            };
            ...
            this.speedY += this.gravity; /*Каждый тик увеличиваем скорость по Y, чтобы при применении этой скорости к
            игроку двигать его вниз.*/
            ...
            if (this.speedY > 0) { // Если скорость по Y больше 0,
                ...
                this.speedY = Math.ceil(this.speedY); // Если скорость по Y меньше или равна 0, то округляем скорость по
                // Y сверху.
            };
